...
func (s *wcaSession) SetMute(t bool) error {
	if err := s.volume.SetMute(t, s.eventCtx); err != nil {
	...
	s.logger.Debugw("Adjusting session mute", "to", fmt.Sprintf("%t", t))
...
func (s *masterSession) SetMute(t bool) error {
	if s.stale {
		s.logger.Warnw("Session expired because default device has changed, triggering session refresh")
		return errRefreshSessions
	}
